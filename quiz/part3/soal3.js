...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++){
    ...
    for (let j = 0; j < shoppers.length; j++){
        if (shoppers[j].product === 'Sepatu Stacattu' && shoppers[j].amount <= hasil[0].leftOver){
            hasil[0].leftOver -= shoppers[j].amount
            hasil[0].shoppers.push(shoppers[j].name)
            hasil[0].totalProfit += (shoppers[j].amount * hasil[0].harga)
        };
        if (shoppers[j].product === 'Baju Zoro' && shoppers[j].amount <= hasil[1].leftOver){
            hasil[1].leftOver -= shoppers[j].amount
            hasil[1].shoppers.push(shoppers[j].name)
            hasil[1].totalProfit += (shoppers[j].amount * hasil[1].harga)
        };
        if (shoppers[j].product === 'Sweater Uniklooh' && shoppers[j].amount <= hasil[2].leftOver){
            hasil[2].leftOver -= shoppers[j].amount
            hasil[2].shoppers.push(shoppers[j].name)
            hasil[2].totalProfit += (shoppers[j].amount * hasil[2].harga)
        };
        
    };
    for (let x = 0; x < hasil.length; x++){
        delete hasil[x].harga
    }
    return hasil
}
